...
path ="data"
data = ImageDataBunch.from_csv(path, ds_tfms=get_transforms(), folder='cars_train', csv_labels='train.csv',  label_col=5, valid_pct=0.2, size=224, num_workers=8, bs=32).normalize(imagenet_stats)
...
train = Trainer(model, criterion, optimizer, train_loader, valid_loader,
        div_lr=3)
train.train_one_cycle(20)
train.model.unfreeze()
train.change_lr(3e-3)
train.train_one_cycle(40)
# train.load_checkpoint('best_model')
# train._validate_epoch()
